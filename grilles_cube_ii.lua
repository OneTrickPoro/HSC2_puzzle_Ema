puzzles:add{
  ...
  build = function (self)
    ...
    self.axes:add(sym:orbit(sym.oox.unit),{1,0.763,-0.763,-1})

    for _, ax, tw in sym:orbit(self.axes[1], rot{from ='x', to = 'y'}) do
      self.twists:add(ax,tw, { gizmo_pole_distance = 1 })
    end

    self.axes:add(sym:orbit(sym.xoo.unit),{ 1.0232,1.454,1.732})
  ...
  tags = {
    builtin = '2.0.0',
    external = { pCubes = 'cercare', '!hof', '!mc4d', museum =6789, '!wca' },
